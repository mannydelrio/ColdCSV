    ...
    <div className="min-h-screen bg-[#FAFAF8] flex flex-col items-center justify-center px-4">
      <div className="w-full max-w-sm">
          ...
          {sent ? (
            <div className="text-center">
              ...
              <p className="text-[#5A5A54] text-sm">
                We sent a magic link to <strong>{email}</strong>. Click it to activate your account and get 10 free credits.
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#2A6B4A] text-white py-2.5 rounded-pill text-sm font-medium hover:bg-[#235c3f] transition disabled:opacity-60"
              >
                {loading ? "Sending..." : "Get started free"}
              </button>

              <p className="text-center text-xs text-[#9A9A94]">
                10 free credits included. No credit card needed.
              </p>
            </form>
          )}
        </div>
        ...
        <p className="mt-6 text-center text-sm text-[#5A5A54]">
          ...
          <Link href="/login" className="text-[#2A6B4A] font-medium hover:underline">
            Sign in
          </Link>
        </p>
      </div>
